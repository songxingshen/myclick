...
EXTRA_PROVIDES =
PROVISIONS = i686 i386 i586 analysis int64 ip6 linux umultithread $(EXTRA_PROVIDES)
...
config.status: $(srcdir)/configure
	$(SHELL) $(srcdir)/configure  '--disable-linuxmodule' '--enable-user-multithread' '--enable-select=select' '--enable-schedule-debugging' '--enable-local' '--enable-ip6'
