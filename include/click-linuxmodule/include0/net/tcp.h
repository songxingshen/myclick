...
/* from /lib/modules/2.6.27.5-117.fc10.i686/build/include/net/tcp.h */
/*
...
static inline void tcp_insert_write_queue_before(struct sk_buff *new_value,
						  struct sk_buff *skb,
						  struct sock *sk)
{
	__skb_insert(new_value, skb->prev, skb, &sk->sk_write_queue);

	if (sk->sk_send_head == skb)
		sk->sk_send_head = new_value;
}
...
static inline void tcp_highest_sack_combine(struct sock *sk,
					    struct sk_buff *old,
					    struct sk_buff *new_value)
{
	if (tcp_sk(sk)->sacked_out && (old == tcp_sk(sk)->highest_sack))
		tcp_sk(sk)->highest_sack = new_value;
}
